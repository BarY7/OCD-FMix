...
batch=None, loss_fn=None,std=0,dopt=0):
    model = copy.deepcopy(bmodel)
    r = copy.deepcopy( model.get_parameter(named_parameter+'.weight').data)
    predicted_labels_base,h = model(batch['input'])
    loss = loss_fn(predicted_labels_base, batch['output'].long())
    lbase = loss.item()
    model.get_parameter(named_parameter+'.weight').data += dopt.squeeze()
    ...
    loptimal = loss.item()
    model.get_parameter(named_parameter+'.weight').data = r + std*w.squeeze().to('cuda')
    predicted_labels_diff,h = model(batch['input'])
    loss = loss_fn(predicted_labels_diff, batch['output'].long())
    ldiffusion = loss.item()
    del model
    return ldiffusion,loptimal,lbase,predicted_labels_diff,predicted_labels_base
...
def generalized_steps(named_parameter, numstep, x, model, bmodel, batch, loss_fn, std, padding, mat_shape, isnerf=0, **kwargs):
    with torch.no_grad():
        ...
        wdiff = xs[-1]
        ldiffusion,loptimal,lbase, predicted_diff, predicted_base = check_ps_wrapper(isnerf=isnerf,named_parameter=named_parameter,
            bmodel=bmodel, w=wdiff.squeeze(), batch=batch,
            loss_fn=loss_fn,std=std,dopt=dopt
            )
    return ldiffusion,loptimal,lbase,wdiff,predicted_diff, predicted_base
